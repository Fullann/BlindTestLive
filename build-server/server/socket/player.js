...
const playerJoinSchema = zod_1.z.object({
...
function registerPlayerHandlers(ctx) {
    ...
    const buzzAttemptsByPlayer = new Map();
    const pushGameLog = (gameId, type, message) => {
    ...
    socket.on("player:joinGame", (rawPayload, callback) => {
        ...
        const newPlayer = {
            ...
            lockedOut: false,
            team: sanitizedTeam,
            playerSecret: playerSecret || crypto_1.default.randomUUID(),
            combo: 0,
    ...
    socket.on("player:buzz", ({ gameId, playerId }, callback) => {
        const now = Date.now();
