...
function computeBadges(totalSessions: number, totalCorrect: number, totalWrong: number): string[] {
  const badges: string[] = [];
  if (totalSessions >= 5) badges.push('habitué');
  if (totalSessions >= 20) badges.push('vétéran');
  if (totalCorrect >= 50) badges.push('oreille d\'or');
  if (totalWrong <= 5 && totalSessions >= 3) badges.push('sniper');
  return badges;
}
...
router.post('/claim', async (req: Request, res: Response) => {
  try {
    ...
    await pool.query(
      `INSERT INTO player_profile_sessions
         (id, public_id, game_id, player_name, score, buzzes, correct_answers, wrong_answers, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [uuidv4(), String(publicId).slice(0, 64), String(gameId).slice(0, 10), safePlayerName, safeScore, safeBuzzes, safeCorrect, safeWrong, now],
    );

    const [profileRows] = await pool.query('SELECT * FROM player_profiles WHERE public_id = ?', [String(publicId).slice(0, 64)]);
    const existing = (profileRows as any[])[0];
    if (!existing) {
      ...
      await pool.query(
        `INSERT INTO player_profiles
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [String(publicId).slice(0, 64), safeNickname, JSON.stringify(badges), JSON.stringify(seasons), 1, safeScore, safeBuzzes, safeCorrect, safeWrong, now, now],
      );
    } else {
      ...
      let seasons: Array<{ id: string; sessions: number; score: number }> = [];
      try {
      ...
      await pool.query(
        `UPDATE player_profiles
         ...
         WHERE public_id = ?`,
        [safeNickname, JSON.stringify(badges), JSON.stringify(seasons), nextSessions, nextScore, nextBuzzes, nextCorrect, nextWrong, now, String(publicId).slice(0, 64)],
      );
    }
...
router.get('/:publicId', async (req: Request, res: Response) => {
  try {
    await ensureSchema();
    const [rows] = await pool.query('SELECT * FROM player_profiles WHERE public_id = ?', [req.params.publicId]);
...
router.get('/:publicId/history', async (req: Request, res: Response) => {
  try {
    await ensureSchema();
    const [rows] = await pool.query(
      `SELECT game_id, player_name, score, buzzes, correct_answers, wrong_answers, created_at
